<template>
  <div style="display: inline-block">
    ...
    <q-btn :color="color" @click="openChat = !openChat" v-else-if="type==='3'" clickable no-caps
           class="rounded-sm q-py-md q-px-xl">
  ...
  methods: {
    ...
    initPusher() {
      ...
      this.echo.channel('global')
          .listen(`.conversation_${this.conversationId}`, message => {

            if( message.data.userId !== this.$store.state.quserAuth.userData.id ) {
              console.error(message,this.$store.state.quserAuth.userData.id )
