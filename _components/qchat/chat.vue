<template>
  <div style="display: inline-block">
    <q-btn flat icon="far fa-comment-dots" @click="openChat = !openChat" no-caps label="Chatea con la tienda" :color="color"  v-if="$q.platform.is.desktop"/>
    <q-btn flat round dense icon="far fa-comment-dots" @click="openChat = !openChat" color="white" v-else/>

    <q-card  id="qchat" v-bind:class="[!openChat ? 'closeChat':'', minimize?'minimizeChat':'']" style="min-width: 300px;">
      <q-bar>
        <div class="cursor-pointer"> <q-icon name="comment" /> {{storeData.name}}</div>
        <q-space />
      ...
      <q-card-section>
        <div style="width: 100%; max-width: 340px; height: 250px; position: absolute; z-index: 10" class="bg-grey-2" v-if="loading" >
